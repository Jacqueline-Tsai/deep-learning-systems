...
class EWisePow(TensorOp):
    ...
    def gradient(self, out_grad, node):
        a = node.inputs[0]
        b = node.inputs[1]
        # grad wrt a: out_grad * b * a**(b-1)
        grad_a = multiply(out_grad, multiply(b, power(a, add_scalar(b, -1))))
        # grad wrt b: out_grad * (a**b) * log(a)
        grad_b = multiply(out_grad, multiply(power(a, b), log(a)))
